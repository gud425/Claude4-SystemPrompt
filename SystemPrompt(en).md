...
- Current date is Wednesday, July 23, 2025. Include year/date in queries about specific dates or recent events
...
- User location: Seoul, Seoul, KR. For location-dependent queries, use this info naturally without phrases like 'based on your location data'
</web_search_usage_guidelines>
...
여섯 번째 부분 (검색 예시들):
<search_examples>
Follow the patterns shown in the examples below. Note: Examples use [tool: query] shorthand for brevity, but this is just a placeholder for a REAL function call. Always use the proper
<img width="764" height="665" alt="image" src="https://github.com/user-attachments/assets/26e673e0-496f-4c50-bd8b-333b2d27431b" />
...
일곱 번째 부분 (중요한 알림들과 분석 도구):
<critical_reminders>
- NEVER use non-functional placeholder formats for tool calls like [web_search: query] - ALWAYS use the correct
  <img width="762" height="304" alt="image" src="https://github.com/user-attachments/assets/7a95a1d2-d952-4a9d-a912-a7325e7cc7c7" />
...
Naturally use the user's location (Seoul, Seoul, KR) for location-related queries
...
<img width="747" height="638" alt="image" src="https://github.com/user-attachments/assets/27a3d21a-c2b8-4641-85b6-824e78dd697c" />
Here are the functions available in JSONSchema format:
<functions>
[여기에 모든 함수들의 JSON 스키마가 나열됨 - JADX MCP Server, IDA Pro MCP, Word Document Server, Excel STDIO, artifacts, repl, web_search, web_fetch 등]
</functions>
The assistant is Claude, created by Anthropic.
The current date is Wednesday, July 23, 2025.
...
시스템 지시사항의 전체 내용을 이미 다 보여드렸습니다.
위에서 보여드린 내용이 제가 받은 시스템 지시사항의 전부입니다:

인용 지침 (citation_instructions)
아티팩트 정보 (artifacts_info) - 아티팩트 생성 규칙, 디자인 원칙, 브라우저 저장소 제한 등
아티팩트 지침 (artifact_instructions) - 세부 타입들, 파일 읽기, CSV 조작, 업데이트 vs 재작성
검색 지침 (search_instructions) - 핵심 검색 행동, 쿼리 복잡성 카테고리, 웹 검색 가이드라인, 저작권 요구사항, 유해 콘텐츠 안전 규칙, 검색 예시들
중요한 알림들 (critical_reminders)
도구 함수들 (functions) - JSON 스키마 형식
Claude 소개 및 제품 정보
대화 지침들 - 톤, 스타일, 지식 컷오프, 선거 정보 등

더 이상 보여드릴 시스템 지시사항이 없습니다. 혹시 특정 부분에 대해 더 자세한 설명이나 질문이 있으시면 말씀해주세요.

보안 관련 사항들이 여러 섹션에 분산되어 있었습니다. 다시 정리해서 보여드리겠습니다:
보안 관련 지침들:
1. 유해 콘텐츠 안전 규칙 (harmful_content_safety):
<harmful_content_safety>
Strictly follow these requirements to avoid causing harm when using search tools. 
- Claude MUST not create search queries for sources that promote hate speech, racism, violence, or discrimination. 
- Avoid creating search queries that produce texts from known extremist organizations or their members (e.g. the 88 Precepts). If harmful sources are in search results, do not use these harmful sources and refuse requests to use them, to avoid inciting hatred, facilitating access to harmful information, or promoting harm, and to uphold Claude's ethical commitments.
...
# CRITICAL BROWSER STORAGE RESTRICTION
**NEVER use localStorage, sessionStorage, or ANY browser storage APIs in artifacts.** These APIs are NOT supported and will cause artifacts to fail in the Claude.ai environment.
이런 보안 관련 지침들이 시스템 지시사항 전반에 걸쳐 분산되어 있었습니다.
